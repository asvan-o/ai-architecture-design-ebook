...
- 각 PROJECT 최상위에 새로 들어온 파일과 지원되는 자산 폴더 패키지를 실시간 감지합니다.
- 개별 파일은 확장자 규칙으로 이동하고, 폴더 패키지는 내부 구조를 유지한 채 폴더 전체를 이동합니다.
...
>
> PROJECT 최상위는 새로 받은 파일 또는 지원되는 자산 폴더 패키지를 임시로 넣는 투입 영역입니다. 교육용 테스트 자료로 먼저 작동을 확인하세요.
...
PROJECT는 Windows 탐색기에서 ROOT 안에 직접 만들거나 프로그램의 `+ 새 프로젝트` 버튼으로 만듭니다. PROJECT 최상위 자체가 신규 파일·폴더 패키지 투입 영역이며 별도의 `_INBOX` 폴더는 만들지 않습니다.
...
4. Windows 탐색기에서 `ROOT → 프로젝트명` 폴더 최상위에 새로 받은 파일 또는 지원되는 자산 폴더 패키지를 넣습니다.
...
- 최소화(`-`): 창만 숨겨지고 파일·폴더 패키지 감시는 계속됩니다.
- 닫기(`X`): watcher와 Electron 프로세스를 완전히 종료합니다.
- 재실행: 꺼져 있던 동안 PROJECT 최상위에 들어온 미분류 파일과 지원되는 폴더 패키지를 먼저 스캔한 뒤 실시간 감시를 시작합니다.
...
- 이름 충돌, 권한 오류, 서로 다른 작업 프로그램 파일이 섞인 폴더와 자동 이동하면 안 되는 폴더형 프로젝트는 `98_REVIEW`에서 확인합니다.

## 폴더형 자산 자동 정리

FBX와 Texture, OBJ·MTL과 Texture처럼 함께 사용하는 파일이 한 폴더에 들어오면 내부 파일을 분리하지 않습니다. 복사가 끝날 때까지 기다린 뒤 폴더 전체를 하나의 자산으로 이동합니다.
...
- MAX·MAYA·BLEND·RHINO·C4D·HOUDINI·Twinmotion 작업파일이 있으면 해당 `04_3D_WORKFILES` 폴더로 패키지 전체를 이동합니다.
- 편집용 작업파일 없이 FBX·OBJ·GLB 등의 교환 모델이 있으면 `06_ASSETS/MODEL`로 이동합니다.
- 이미지만 있는 텍스처 묶음은 `06_ASSETS/IMAGE`, 영상만 있는 묶음은 `06_ASSETS/VIDEO`, 문서만 있는 묶음은 `07_DOCUMENT`로 이동합니다.
- 서로 다른 주 작업파일이 섞이거나 구성이 명확하지 않으면 폴더 전체를 `98_REVIEW`로 보내 사람이 확인합니다.
- 같은 이름의 대상 폴더가 이미 있으면 덮어쓰기나 병합을 하지 않고 새 폴더 전체를 `98_REVIEW/충돌-날짜시간`으로 이동합니다.
- 폴더 안의 파일명·하위 폴더명·Material·Texture·참조 경로는 바꾸지 않습니다.

폴더 복사 중에는 500ms 간격으로 재귀 파일 수와 전체 크기, 최근 수정 상태를 확인합니다. 3회 연속 변화가 없을 때만 이동합니다. 계속 변하거나 사용 중인 폴더는 강제로 옮기지 않고 대기 상태로 남깁니다.
...
- 프로그램은 CAD Xref, Revit Link, SketchUp 외부 파일, 3D Texture, Houdini 외부 참조를 수정하지 않습니다. 폴더 내부 구조를 보존해도 폴더 밖의 파일을 참조하면 링크가 끊어질 수 있으므로 외부 참조가 연결된 작업파일 세트는 자동정리 전에 확인합니다.
